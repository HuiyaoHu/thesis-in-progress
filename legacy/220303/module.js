...
var matLineAttr = new THREE.LineBasicMaterial( {color: 0x0000ff, linewidth: 1} );
...
loader.load( // Load a glTF resource
    'models/LineTest.gltf', // resource URL
    function ( gltf ) { // called when the resource is loaded
        lineAttr = gltf.scene;
        lineAttr.position.set(2,0,0)
        
            console.log(lineAttr) // Group {uuid: '506B1222-C3A8-4B73-8693-3EB2EBF8AEB4', name: 'Scene', type: 'Group', parent: null, children: Array(1), …}
            console.log(lineAttr.children) // [Object3D] > 0: Object3D {uuid: 'D03E136E-BBA6-4087...
            console.log(lineAttr.children[0].children) // (4) [Line, Line, Line, Line]
            console.log(lineAttr.children[0].children[0]) // Line {uuid: '348AFFFF-F9F0-4944-A895-654FD47747E1', name: 'attribute_line', type: 'Line', parent: Object3D, children: Array(0), …}
        
        var line0 = lineAttr.children[0].children[0]
        line0.material = matLineAttr;
        
            console.log(line0)
            console.log(line0.geometry.attributes.position) // BufferAttribute {name: '', array: Float32Array(6), itemSize: 3, count: 2, normalized: false, …}
            console.log(line0.geometry.attributes.position.array[0]) // -1.7500005960464478
        
        console.log(line0.geometry.attributes.position.array) // Float32Array(6) [-1.7500005960464478, 0, -1.7500011920928955, 1.7500005960464478, 0, -1.7500011920928955, buffer: ArrayBuffer(24), byteLength: 24, byteOffset: 0, length: 6, Symbol(Symbol.toStringTag): 'Float32Array']
        var arr_line0 = line0.geometry.attributes.position.array

        var dotMaterial = new THREE.PointsMaterial( { size: 1, color: "pink"} );

        var dotGeometry3 = new THREE.Geometry();
        dotGeometry3.vertices.push( getCenterPoint(line0) );
        var dot3 = new THREE.Points( dotGeometry3, dotMaterial );
        scene.add( dot3 );
        ...
        function getCenterPoint(mesh) {
            ...
            var pos1_line0 = new THREE.Vector3( arr_line0[0], arr_line0[2], arr_line0[1])
            var dotGeometry1 = new THREE.Geometry();
            dotGeometry1.vertices.push( pos1_line0 );
            var dot1 = new THREE.Points( dotGeometry1, dotMaterial );
            scene.add( dot1 );

            var pos2_line0 = new THREE.Vector3( arr_line0[3], arr_line0[5], arr_line0[4])
            var dotGeometry2 = new THREE.Geometry();
            dotGeometry2.vertices.push( pos2_line0 );
            var dot2 = new THREE.Points( dotGeometry2, dotMaterial );
            scene.add( dot2 );
            
        
            
            lineAttr.traverse(function(child) { // returns all children of objects with a matching name.
                if (child.name === "attribute_line") {
                child.material = matLineAttr;
                }
            });
            
        lineAttr.name = "LineTest";
        scene.add( lineAttr );
    },
...
function creatingScene() {
    ...
    // const line = new THREE.LineSegments( edges, new THREE.LineBasicMaterial( { color: 0x888 } ) );
    // scene.add( line );
...
const dotMaterial = new THREE.PointsMaterial( { size: 1, color: "pink"} );
const dotMaterial_Large = new THREE.PointsMaterial( { size: 2, color: "pink"} );
const top_left_corner = new THREE.Vector3( -volume_width*2, volume_width*2, 0 );
const matText_Attr = new THREE.LineBasicMaterial( {color: 0x0000ff, linewidth: 1} );


// Display Starting Point
dispDotsfromCoords (dotMaterial_Large, [top_left_corner]);
...
function draw_AttrLine (first_pt, ...args) { // 'RRR DDD LLL UUUU'
    var start_pt = first_pt; // to be overwritten 
    var AttrLine = null; // to be overwritten

    var list_attrSet = Array.from(arguments); // e.g. list_attrSet = [ first_pt, ['R', 'corridor'], ['D', 'corridor'] ]
    list_attrSet.shift(); // return array with the first item removed

    list_attrSet.forEach ( function(attrSet) { // e.g. attrSet = ['R', 'corridor']


        [start_pt, AttrLine] = drawCmd_AttrLines ( start_pt, attrSet[0] ); // input: start_point, command; output: next start_point, added line with dots to the scene
        console.log(start_pt)

        // Adding Attributes
        AttrLine.name = "attribute_line";
        AttrLine.adjacency = attrSet[1];
        
        // Add Text
        const textDisp_attr = String( attrSet );
        addAttrText (textDisp_attr, start_pt); // toggle for display
...
// Functions
function addAttrText (textDisp_attr, pos_text) {
    fontloader.load('https://unpkg.com/three@0.119.1/examples/fonts/helvetiker_regular.typeface.json', function(font) {
        var textgeometry = new THREE.TextGeometry(textDisp_attr, {
        font: font,
        size: 0.2,
        height: 0,
        });
        // geometry.center();
        const textmesh = new THREE.Mesh(textgeometry, matText_Attr);
        textmesh.position.copy(pos_text);
        scene.add(textmesh);
    });
}


function drawCmd_AttrLines (start_pt, dir) { // based on command, execute function accordingly
    var AttrLine = null; // to be overwritten
    if (dir == 'R') {
        [start_pt, AttrLine]= drawAttrLineRight (start_pt);
    };
    if (dir == 'D') {
        [start_pt, AttrLine] = drawAttrLineDown (start_pt);
    };
    if (dir == 'L') {
        [start_pt, AttrLine] = drawAttrLineLeft (start_pt);
    };
    if (dir == 'U') {
        [start_pt, AttrLine] = drawAttrLineUp (start_pt);
    };
    return [start_pt, AttrLine]
}

function drawAttrLineRight (start_pt) {
    const end_pt = start_pt.clone().add( new THREE.Vector3( volume_width, 0, 0 ) );
    const AttrLine = draw_AttrLineUnit (start_pt, end_pt, lineMaterial, dotMaterial);
    return [end_pt, AttrLine]
}
function drawAttrLineDown (start_pt) {
    const end_pt = start_pt.clone().add( new THREE.Vector3( 0, -volume_width, 0 ) );
    const AttrLine = draw_AttrLineUnit (start_pt, end_pt, lineMaterial, dotMaterial);
    return [end_pt, AttrLine]
}
function drawAttrLineLeft (start_pt) {
    const end_pt = start_pt.clone().add( new THREE.Vector3( -volume_width, 0, 0 ) );
    const AttrLine = draw_AttrLineUnit (start_pt, end_pt, lineMaterial, dotMaterial);
    return [end_pt, AttrLine]
}
function drawAttrLineUp (start_pt) {
    const end_pt = start_pt.clone().add( new THREE.Vector3( 0, volume_width, 0 ) );
    const AttrLine = draw_AttrLineUnit (start_pt, end_pt, lineMaterial, dotMaterial);
    return [end_pt, AttrLine]
}
...
function draw_AttrLineUnit (starting_pt, ending_pt, lineMaterial, dotMaterial) {
    const points = [];
    ...
    var dot2 = new THREE.Points( geometry, dotMaterial );
...
/* Discarded
    ...
    // var end_pt = drawAttrLineRight( top_left_corner );
    // drawAttrLineRight( end_pt );

    // const pos_attributeLine_1 = top_left_corner.clone().add( new THREE.Vector3( volume_width_half, 0, 0 ) );
    // const pos_attributeLine_2 = pos_attributeLine_1.clone().add( new THREE.Vector3( volume_width, 0, 0 ) );

    // const list_pos_attributeLine = [pos_attributeLine_1, pos_attributeLine_2];
    // drawAttrLine( list_pos_attributeLine );

    // Create Unit of Attribute Line
    const unit_starting_pt = new THREE.Vector3(-volume_width_half, 0, 0);
    const unit_ending_pt = new THREE.Vector3(volume_width_half, 0, 0);

    const grp_lineWithDots = draw_lineWithDots (unit_starting_pt, unit_ending_pt, material);


function drawAttrLine(list_coord) {
    list_coord.forEach( function(coord){
        var group_clone = grp_lineWithDots.clone();
        scene.add(group_clone);
        group_clone.position.copy(coord);
        group_clone.name = "attribute_line";
        group_clone.ajacency = "corridor";
    } ) 
}

function create_list (){
    var list_coord = Array.from(arguments)

    const list_pos_attributeLine = []
    list_coord.forEach( function(coord){
        list_pos_attributeLine.push( coord );
    } ) 
    return list_pos_attributeLine
}    

function draw_AttrLine () {
    var list_dir = Array.from(arguments)
    list_dir.forEach( function( dir ) {
        if (dir == 'L') {
            // continueAttrLineLeft (prev_pos)
        };
        if (dir == 'R') {
            drawAttrLineRight (prev_pos);
        };
    });
}
