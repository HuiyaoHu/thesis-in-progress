...
// Define Variables
var camera, scene, renderer,  raycaster, ground,
    dir_light1, dir_light1_helper, dir_light2, dir_light2_helper, dir_light1ShadowHeper, hemi_light, hemi_light_helper, 
    wall_trans, wall_trans_del, floor_trans, floor_trans_del, Window01_trans, Window01_trans_del, Door01_trans, Door01_trans_del;
...
function onMouseDown(event) {
    mouse_down.x = event.clientX; // e.g. 1
    mouse_down.y = event.clientY // e.g. 1
}
...
function onMouseUp(event) { // Mouse up: do nothing, create mesh or delete mesh
    ...
    // ‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾
        ...
        if (del_floor && key in floors) { // if shift is pressed and existing key is True, delete floor
            ...
            console.log('~~~floors[key]==undefined~~~', floors[key]==undefined) 
            addFloor(key); 
        }
    }
...
function render() {
    ...
    if (button_index == 0){
        ...
        if ( intersects.length > 0 ) { // if intersect with an object
            ...
            if (isect0.object.name == 'floor') { // if the first object that the ray intersects has the name "floor"
                var floor_cen = isect0.object.position; // centre of the first object that the ray intersects, e.g. Vector3 {x: -1.5, y: 3, z: 0.25}
                floor_pos = new THREE.Vector3(floor_cen.x, floor_cen.y, floor_cen.z); // update global variable floor_pos, e.g. Vector3 {x: -1.5, y: 3, z: 0.25}
    ...
    if (button_index == 1){
        ...
        if ( intersects.length > 0 ) { // if intersect with an object
            ...
            else if (isect0.object.name == 'Window01') { // if the first object that the ray intersects has the name ' '
            ...
            else if (isect0.object.name == 'Door01') { 
                if (!del_wall) {
                    var wall_cen = isect0.object.position;
                    wall_pos = new THREE.Vector3(wall_cen.x, wall_cen.y, wall_cen.z);
                    wall_rotation = isect0.object.rotation.z;
    ...
    if (button_index == 2){
        ...
        isect_objs = isect_objs.concat(Object.values(Door01s));
        isect_objs.push(ground);
        var intersects = raycaster.intersectObjects( isect_objs, true );
        var isect0 = intersects[ 0 ];
        ...
        if ( intersects.length > 0 ) { // if intersect with an object
            ...
            if (isect0.object && isect0.object.parent && isect0.object.parent.parent && isect0.object.parent.parent.name == 'Window01') { // if the first object that the ray intersects has the name ' '
                var isectObj = isect0.object.parent.parent
                var Window01_cen = isectObj.position;
    ...
    if (button_index == 3){
        ...
        if ( intersects.length > 0 ) {  // if intersect with an object
            ...
            else if (isect0.object.name == 'Window01') { 
                if (!del_Door01) {
                    var Door01_cen = isect0.object.position;
                    Door01_pos = new THREE.Vector3(Door01_cen.x, Door01_cen.y, Door01_cen.z);
                    Door01_rotation = isect0.object.rotation.z;
...
function addDoor01(key) {
    ...
    document.getElementById(button_ids[button_index]).innerHTML = "Door01: " + Door01_counters[Door01.button_index];
}
