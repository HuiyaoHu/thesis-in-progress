...
var matHybridTrans = new THREE.MeshStandardMaterial({color: 'green', opacity: 0.2, transparent: true});
var matBdyWallTrans = new THREE.MeshStandardMaterial({color: 'burlywood', opacity: 0.5, transparent: true});
var matBdyWallDel = new THREE.MeshStandardMaterial({color: 'red', opacity: 0.2, transparent: true});
...
function creatingScene() {
    ...
    container = document.getElementById("centre_panel");
    ...
    meshWindow01Trans = new THREE.Mesh( geomWindow01Hover, matHybridTrans );
    ...
    meshWindow02Trans = new THREE.Mesh( geomWindow01Hover, matHybridTrans );
    ...
    meshDoor01Trans = new THREE.Mesh( geomDoor01Hover, matHybridTrans );
    ...
    meshDoor02Trans = new THREE.Mesh( geomDoor01Hover, matHybridTrans );
    ...
    meshDoor03Trans = new THREE.Mesh( geomDoor01Hover, matHybridTrans );
    ...
    meshRailing01Trans = new THREE.Mesh( geomRailing01Hover, matHybridTrans );
    ...
    meshStairs01Trans = new THREE.Mesh( geomStairs01Hover, matHybridTrans );
    ...
    addAttrLine_availVolBdy ( // Add Attribute line
        top_left_corner,
        ['R', 'corridor'],
        ['R', 'corridor'],
        ['R', 'corridor'],
...
// console.log(meshNeighbour01)
        ...
        if ( list_meshInt.length > 0 ) { // if intersect with any mesh
            ...
            if (meshInt0.object.parent.name == 'Window01') { // if the first mesh that the cursor intersects has the name ' '
               ...
               if (!bool_delWindow01) { // if shift button is not pressed, update global variable of geom & geom_trans
                    ...
                    const grpAttrLine = getAttriLine_atBdyWall( meshInt );
                    if (grpAttrLine != undefined) { // if attribute line exisit
                        replaceHoverDisp_baseOnAttr ( replaceHoverDisp_Window01, grpAttrLine, meshInt, 'neighbour', 'extent' )
                    } else { // if meshInt is not on an attribute line
                        replaceHoverDisp_Window01( meshInt );
                    }
        ...
        if ( list_meshInt.length > 0 ) {  // if intersect with any meshes
            ...
            if (meshInt0.object.parent.name == 'Window02') { // if the first mesh that the cursor intersects has the name ' '
                ...
                if (!bool_delWindow02) { // if shift button is not pressed, update global variable of geom & geom_trans
                    ...
                    const grpAttrLine = getAttriLine_atBdyWall( meshInt );
                    if (grpAttrLine != undefined) { // if attribute line exisit
                        replaceHoverDisp_baseOnAttr ( replaceHoverDisp_Window02, grpAttrLine, meshInt, 'neighbour', 'extent')
                    } else { // if meshInt is not on an attribute line
                        replaceHoverDisp_Window02( meshInt );
                    }
    ...
    if ( id_buttonPressed == 'buttonDoor01' ) {
        ...
        if ( list_meshInt.length > 0 ) {  // if intersect with any meshes
            // const meshInt = meshInt0.object;
            // const angle_meshInt = meshInt.object.rotation.z;
            // const pos_adjFloor_front = 
            // // console.log(angle_meshInt)
            // if ( angle_meshInt == 0 || angle_meshInt == Math.PI || angle_meshInt == - Math.PI ) { // meshInt facing left or right
            //     console.log("facing front or back");

            // } else { // meshInt facing front or back
            //     console.log("facing left or right");
            // }

            // function checkPresence_adjacentFloor () {
            ...
            //OTHER PREREQUISITE
                ...
                if (!bool_delDoor01) { // if shift button is not pressed, update global variable of geom & geom_trans
                    ...
                    const grpAttrLine = getAttriLine_atBdyWall( meshInt );
                    if (grpAttrLine != undefined) { // if attribute line exisit
                        replaceHoverDisp_baseOnAttr ( replaceHoverDisp_Door01, grpAttrLine, meshInt, 'neighbour', 'extent')
                    } else { // if meshInt is not on an attribute line
                        replaceHoverDisp_Door01( meshInt );
                    }
        ...
        if ( list_meshInt.length > 0 ) {  // if intersect with any meshes
            ...
            if (meshInt0.object.parent.name == 'Door02') { // if the first mesh that the cursor intersects has the name ' '
                ...
                if (!bool_delDoor02) { // if shift button is not pressed, update global variable of geom & geom_trans
                    ...
                    const grpAttrLine = getAttriLine_atBdyWall( meshInt );
                    if (grpAttrLine != undefined) { // if attribute line exisit
                        replaceHoverDisp_baseOnAttr ( replaceHoverDisp_Door02, grpAttrLine, meshInt, 'neighbour', 'extent')
                    } else { // if meshInt is not on an attribute line
                        replaceHoverDisp_Door02( meshInt );
                    }
        ...
        if ( list_meshInt.length > 0 ) {  // if intersect with any meshes
            ...
            if (meshInt0.object.parent.name == 'Door03') { // if the first mesh that the cursor intersects has the name ' '
                ...
                if (!bool_delDoor03) { // if shift button is not pressed, update global variable of geom & geom_trans
                    ...
                    const grpAttrLine = getAttriLine_atBdyWall( meshInt );
                    if (grpAttrLine != undefined) { // if attribute line exisit
                        replaceHoverDisp_baseOnAttr ( replaceHoverDisp_Door03, grpAttrLine, meshInt, 'neighbour', 'extent')
                    } else { // if meshInt is not on an attribute line
                        replaceHoverDisp_Door03( meshInt );
                    }
        ...
        if ( list_meshInt.length > 0 ) {  // if intersect with any meshes
            ...
            if (meshInt0.object.parent.name == 'Railing01') { // if the first mesh that the cursor intersects has the name ' '
                ...
                if (!bool_delRailing01) { // if shift button is not pressed, update global variable of geom & geom_trans
                    ...
                    const grpAttrLine = getAttriLine_atBdyWall( meshInt );
                    if (grpAttrLine != undefined) { // if attribute line exisit
                        replaceHoverDisp_baseOnAttr ( replaceHoverDisp_Railing01, grpAttrLine, meshInt, 'neighbour', 'extent')
                    } else { // if meshInt is not on an attribute line
                        replaceHoverDisp_Railing01( meshInt );
                    }
...
function getbool_keyExistance(key_mesh) {
    var bool_keyExistance = dictAttrLine [key_mesh] !==undefined || dictBdyWall [key_mesh] !==undefined || dictPartWall [key_mesh] !==undefined || dictWindow01 [key_mesh] !==undefined || dictWindow02 [key_mesh] !==undefined || dictDoor01 [key_mesh] !==undefined || dictDoor02 [key_mesh] !==undefined || dictDoor03 [key_mesh] !==undefined || dictRailing01 [key_mesh] !==undefined || dictStairs01 [key_mesh] !==undefined;
...
function replaceHoverDisp_baseOnAttr (func_replaceHoverDisp, grpAttrLine, meshtoReplace, ...args) {

    var list_args = Array.from(arguments);
    const args_adjacency = list_args.slice(3) // remove the first 3 parameters

    if (grpAttrLine.adjacency == 'neighbour') { // check attribute
        showHelper("Only wall modules can be placed against boundaries with neighouring volumes.")
    } else if (grpAttrLine.adjacency == 'extent') {
        showHelper("Only wall modules can be placed against boundaries with neighouring volumes.")
    } else {
...
// --------------------------------
//    Match Key to Boundary Wall 
// --------------------------------


function getAttriLine_atBdyWall( meshBdyWall ) {
    const pos_meshInt = meshBdyWall.position;
    const angle_meshInt = meshBdyWall.rotation.z;
    const pos_grpAttrLine = getPos_AttrLine_atBdyWall (pos_meshInt, angle_meshInt);
    const key_grpAttrLine = keyGen( pos_grpAttrLine );
    const grpAttrLine = dictAttrLine[key_grpAttrLine]
    return grpAttrLine
}

function getPos_AttrLine_atBdyWall (pos_BdyWall, angle_BdyWall) { 
    var pos_intAttrLine = null;
    if (angle_BdyWall == Math.PI / 2 ) { // left boundary wall
        var pos_intAttrLine = new THREE.Vector3 ( (pos_BdyWall.x - offsetValue_BdyWall) , pos_BdyWall.y, (pos_BdyWall.z-BdyWall_height_half) );
    } else if ( angle_BdyWall == - Math.PI / 2 ) { // right boundary wall
        var pos_intAttrLine = new THREE.Vector3 ( (pos_BdyWall.x + offsetValue_BdyWall) , pos_BdyWall.y, (pos_BdyWall.z-BdyWall_height_half) );
    } else if ( angle_BdyWall == 0 ) { // front boundary wall
        var pos_intAttrLine = new THREE.Vector3 ( pos_BdyWall.x, (pos_BdyWall.y + offsetValue_BdyWall), (pos_BdyWall.z-BdyWall_height_half) );
...
function addAttrLine_availVolBdy (first_pt, ...args) { // 'RRR DDD LLL UUUU'
...
function addAttrLineUnit (starting_pt, ending_pt, matAttrLine, matAttrDot, attrSet) {
    const points = [];
    ...
    addAttrText (textDisp_attr, pos_midpoint); // toggle for display
...
function deleteAttrLineUnit (key) {
    scene.remove( dictAttrLine[key] );
    delete dictAttrLine[ key ];
    scene.remove( dictTextMesh[key] );
    delete dictTextMesh[ key ];
}

function addAttrText (textDisp_attr, pos_text) {
    fontloader.load('https://unpkg.com/three@0.119.1/examples/fonts/helvetiker_regular.typeface.json', 
    function(font) {
        ...
        // UPDATE GLOBAL VARIABLES
        const key_textMesh = keyGen(pos_text);
...
function addAttrLine_purchasedVolBdy (list_CoordOfCorner) {
    ...
    for(var i=0; i < list.length - 1; i++){
        ...
        if ( bool_keyExistance ) { // if key already exist
            const grpAttrLine = dictAttrLine[key];
            const adjacency = grpAttrLine.adjacency;
            if (adjacency == 'extent')  {
                deleteAttrLineUnit (key);
...
function addAttrLine_BdyWall (list_CoordOfCorner) {
    const grpAttrLine = getAttriLine_atBdyWall;
    if (grpAttrLine = undefined) { // if it doesnt not exist
