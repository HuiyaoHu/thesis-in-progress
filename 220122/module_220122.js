...
var volume_mat_trans = new THREE.MeshLambertMaterial({color: 'burlywood', opacity: 0.2, transparent: true});
var volume_mat_trans_del = new THREE.MeshLambertMaterial({color: 'white', opacity: 0.6, transparent: true});
...
// GEOMETRIES
var volume_geom = new THREE.BoxBufferGeometry( volume_width, volume_length, volume_height );
var volume_geom_trans = new THREE.BoxBufferGeometry( volume_width, volume_length, volume_height );
var volume_geom_del = new THREE.BoxBufferGeometry( volume_width * 1.1, volume_length* 1.1, volume_height * 1.1 );
...
/*
    ...
    var ground2 = new THREE.Mesh( shape1, volUnit_mat_trans ) ;
...
*/
    ...
    volume_trans = new THREE.Mesh( volume_geom_trans, volume_mat_trans );
    volume_trans.visible = false;
    scene.add(volume_trans);
    volume_trans_del = new THREE.Mesh( volume_geom_del, volume_mat_trans_del );
...
function render() {
    ...
    if ( button_id == 'buttonVolume' ) {
        ...
        var sceneMeshes = Object.values(volumes); // get the meshes in the scene to check for intersections
...
function addVolume(key) {
    ...
    // Add Mesh
    var volume = new THREE.Mesh(volume_geom, volume_mat_trans);
    volume.position.set(volume_pos.x, volume_pos.y, volume_pos.z);
    scene.add( volume );

    // Add Mesh Properties
    volume.name = "volume"
    volume.volume_key = key;
    ...
    // console.log('~~~volume~~~', volume.geometry.attributes.position.array);

    var base_frontleft_cood = new THREE.Vector3( volume_pos.x - volume_width_half, volume_pos.y + volume_width_half, volume_pos.z - volume_width_half );
    // console.log('~~~base_frontleft_cood~~~', base_frontleft_cood);
    addCood(base_frontleft_cood);


    var base_frontright_cood = new THREE.Vector3( volume_pos.x + volume_width_half, volume_pos.y + volume_width_half, volume_pos.z - volume_width_half );
    addCood(base_frontright_cood);

    var base_backright_cood = new THREE.Vector3( volume_pos.x + volume_width_half, volume_pos.y - volume_width_half, volume_pos.z - volume_width_half );
    addCood(base_backright_cood);

    var base_backleft_cood = new THREE.Vector3( volume_pos.x - volume_width_half, volume_pos.y - volume_width_half, volume_pos.z - volume_width_half );
    addCood(base_backleft_cood);
    

    
    // var top_frontleft_cood = new THREE.Vector3( volume_pos.x - volume_width_half, volume_pos.y + volume_width_half, volume_pos.z + volume_width_half );
    // addCood(top_frontleft_cood);

    // var top_frontright_cood = new THREE.Vector3( volume_pos.x + volume_width_half, volume_pos.y + volume_width_half, volume_pos.z + volume_width_half );
    // addCood(top_frontright_cood);

    // var top_backleft_cood = new THREE.Vector3( volume_pos.x - volume_width_half, volume_pos.y - volume_width_half, volume_pos.z + volume_width_half );
    // addCood(top_backleft_cood);

    // var top_backright_cood = new THREE.Vector3( volume_pos.x + volume_width_half, volume_pos.y - volume_width_half, volume_pos.z + volume_width_half );
    // addCood(top_backright_cood);
    ...
    // // var matShape = new THREE.MeshBasicMaterial({color:"blue"});
    // var floorZone = new THREE.Mesh( geomShape, volume_mat_trans ) ;
    // scene.add( floorZone );
...
// function showPoints() {
//     var keys = Object.keys(dictVolume);
//     // console.log(keys)

//     keys.forEach(function(key){
//         console.log(dictVolume[key]);
//         // Visualise Points
//         var geomPoint = new THREE.BufferGeometry().setFromPoints([dictVolume[key]]);
...
function deleteVolume(volume) {
    scene.remove( volume );
    delete volumes[ volume.volume_key ];
    volume_counters -= 1; 
    document.getElementById('buttonVolume').innerHTML = "Volume:" + volume_counters;
};
